...
function parseFrontmatter(content: string) {
  ...
  frontmatterBlock.split("\n").forEach((line) => {
    ...
    if (key && valueParts.length) {
      frontmatter[key.trim()] = valueParts.join(":").trim().replace(/^['"](.*)['"]$/, "$1"); // Remove quotes
    }
  });
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ success: true, slug, post: postData }), { status: 200 });

  } catch (e: any) {
    console.error("Upload Error:", e);
    
    if (e.message.includes("UNIQUE constraint failed")) {
         return new Response(JSON.stringify({ error: "Slug already exists. Change title or slug in frontmatter." }), { status: 409 });
    }
