...
export const GET: APIRoute = async ({ request, locals }) => {
	...
	try {
		...
		const vectorMatches = await querySimilarPosts(
			env.AI,
			env.VECTOR_INDEX,
			q,
			20,
		...
		const validPosts = await db
			...
			.from(posts)
			.where(and(inArray(posts.id, matchIds), isNotNull(posts.publishedAt)))
			.all();

		// 3. Combine Results (Attach scores from Vectorize)
		const results = validPosts
			.map((post: any) => {
			...
			.slice(0, 5); // Return top 5 valid ones
