...
export default function Comments({ postId, currentUser }: CommentsProps) {
    ...
    return (
        <div class="mt-12 border-t border-white/10 pt-8">
            <h3 class="text-2xl font-bold font-display mb-6">Transmissions ({comments.length})</h3>

            {/* Comment Form */}
            {currentUser ? (
                <form onSubmit={handleSubmit} class="mb-8">
                    <div class="flex gap-4">
                        <img src={currentUser.avatar || `https://ui-avatars.com/api/?name=${currentUser.username}`} class="w-10 h-10 rounded-full border border-white/10" />
                        <div class="flex-1">
                            <textarea
                                value={newComment}
                ...
                <div class="bg-white/5 rounded-xl p-6 text-center mb-8">
                    <p class="text-gray-400 mb-4">Identify yourself to join the frequency.</p>
                    <a href="/login" class="px-6 py-2 rounded-full bg-white text-black font-bold text-sm hover:bg-gray-200 transition-colors">Login to Comment</a>
                </div>
            )}

            {/* Comment List */}
            {loading ? (
                <div class="text-center text-gray-500 py-8">Scanning frequencies...</div>
            ) : (
                <div class="space-y-6">
                        ...
                        <div key={comment.id} class="group animate-fade-in">
                            <div class="flex gap-4">
                                <div class="flex-shrink-0">
                                     <img src={comment.user.avatar || `https://ui-avatars.com/api/?name=${comment.user.username}`} class="w-10 h-10 rounded-full border border-white/10" />
                                </div>
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-1">
                                        <span class="font-bold text-white text-sm">{comment.user.username}</span>
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        >
                                            <span class={comment.likedByUser ? 'i-heroicons-heart-solid' : 'i-heroicons-heart'}></span>
                                            {comment.likes}
                                        </button>
                                        <button class="text-xs text-gray-500 hover:text-white transition-colors">Reply</button>
                                    </div>
                                </div>
                            </div>
                        </div>
